...
| **Interactive mode** *(logged in)* | After Round 1, pick Advocate or Critic — agents personalize later rounds to encourage or persuade you |
...
5. **Interactive mode**: after Round 1, debate pauses; user submits stance via `POST /api/debates/<id>/stance/`; `continue_debate` resumes with personalized prompts.
...
| `POST` | `/api/debates/<id>/stance/` | Required | Submit stance: `{ stance: "advocate" \| "critic" }` |
